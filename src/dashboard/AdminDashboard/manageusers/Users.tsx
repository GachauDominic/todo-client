...
const Users = () => {
  const {data: usersData, isLoading, error}= usersAPI.useGetUsersQuery(
    undefined,
    {
      refetchOnMountOrArgChange: true, // Refresh data when component mounts or query args change 
      pollingInterval: 60000, // Poll every 60 sec to keep data fresh
    }
  )
                ...
                usersData.map((user: TUser)=>(
                  <tr key={user.id} className="hover:bg-gray-300 border-b border-gray-400">
                    ...
                    <td className="px-4 py-2">
                      <button className="btn btn-sm btn-primary text-blue-500"
                        onClick={() => {
