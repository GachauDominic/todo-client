import { todoAPI, type TTodo } from "../../../../src/features/todos/todosApi";
...
const UpdateTodo = ({ todo, refetch }: UpdateTodoProps) => {
    const [updateTodo, { isLoading }] = todoAPI.useUpdateTodoMutation({ fixedCacheKey: "updateTodo" });

    if (!todo) return null;

    const handleStatusChange = async () => {
			try {
				const result = await updateTodo({ id: todo.id, isCompleted: !todo.isCompleted }).unwrap();
