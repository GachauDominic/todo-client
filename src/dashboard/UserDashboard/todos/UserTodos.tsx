...
const UserTodos = () => {
  ...
  const { data: todosData, isLoading: todosLoading, error: todoError, refetch: refetchTodos } = todoAPI.useGetTodosByUserIdQuery(
    userId ?? 0, // Use the user ID or 0 if not available
    // userId!, // Get the user ID from the user state
